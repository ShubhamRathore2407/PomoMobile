import { GroupTasksByDateProps, Status, TaskItemProps, TaskType, } from "../../services/models";

export const groupTasksByDate = (tasks: TaskItemProps[]) => {
  const groupedData: GroupTasksByDateProps[] = [];
  tasks.forEach((task) => {
    const existingGroup = groupedData.find((group) => group.date.getDate() === task.activeOn.getDate());
    if (existingGroup) {
...
export const getTodayTasks = (tasks: Realm.Results<TaskItemProps>) => {
  ...
  const todayTasks = tasks.filtered(`activeOn >= $0 AND activeOn < $1`, startOfDay, endOfDay);
  return todayTasks;
};
...
export const sortBasedOnTaskType = (type : TaskType, data: GroupTasksByDateProps[]) => {
  return type === TaskType.CURRENT ? data.sort((a, b) => b.date.getTime() - a.date.getTime()) : data.sort((a, b) => a.date.getTime() - b.date.getTime())
}